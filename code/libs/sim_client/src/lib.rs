...
use sim_protocol::{recv_msg, send_msg, Pose2D, Side, SimMsgC2S, SimMsgS2C};
...
pub struct SimClient {
    ...
    /// Latched starter state — updated by the RX loop on every
    /// `SimMsgS2C::Starter`. The mock starter pin reads this.
    starter_inserted: Arc<AtomicBool>,
    /// Latched side assignment — `true` = Left. Updated on every
    /// `SimMsgS2C::Side`. The mock colour pin reads this.
    side_is_left: Arc<AtomicBool>,
}

impl SimClient {
    /// Whether the simulated starter cable is currently inserted.
    pub fn is_starter_inserted(&self) -> bool {
        self.starter_inserted.load(Ordering::Relaxed)
    }

    /// Whether the sim assigned this robot to the Left side.
    pub fn is_side_left(&self) -> bool {
        self.side_is_left.load(Ordering::Relaxed)
    }
}
...
impl SimClient {
    ...
    pub fn connect(socket_path: &str, hello: SimMsgC2S) -> io::Result<Self> {
        ...
        let starter_inserted = Arc::new(AtomicBool::new(false));
        let side_is_left = Arc::new(AtomicBool::new(true));

        // Spawn RX thread on a clone of the stream.
        let rx_stream = stream.try_clone()?;
        let rx_starter = starter_inserted.clone();
        let rx_side = side_is_left.clone();
        thread::Builder::new()
            .name("sim-rx".into())
            .spawn(move || {
                rx_loop(
    ...
    loop {
        match recv_msg::<_, SimMsgS2C>(&mut r) {
            Ok(msg) => match msg {
                ...
                SimMsgS2C::Starter { inserted } => {
                    s.starter.store(inserted, Ordering::Relaxed);
                }
                SimMsgS2C::Side { side } => {
                    s.side.store(matches!(side, Side::Left), Ordering::Relaxed);
                }
            },
            Err(e) => {

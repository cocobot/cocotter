...

{
    ...
    pub fn new(spi: SPI, address: u8) -> Self {
        Self {
            ...
            angle                : Sch16tAxis { x: 0.0, y: 0.0, z: 0.0 },
            acceleration         : Sch16tAxis { x: 0.0, y: 0.0, z: 0.0 },
    ...
    pub fn init(&mut self) -> Result<(), Sch16tError<SPI>> {
        ...
        println!("test {}", self.read_register(Register::SysTest)?);
        // configure registers
        let ctrl_rate_val = SCH16TRateCtrl{xyz1_scale : self.angle_scale,
        ...
        println!("ctrl rate {:#010x}", self.read_register(Register::CtrlRate)?);

...
impl<SPI> Lsm6dso<SPI>
    ...
    pub fn new(configuration: Lsm6dsoConfiguration, spi_dev: SPI, cs: Pins) -> Self {
        Lsm6dso { 
            spi         : spi_dev,
            cs          : cs,
            dev_conf    : configuration, 
            acceleration: Meas3d::from([0,0, 0,0, 0,0]), 
            angular_rate: Meas3d::from([0,0, 0,0, 0,0]), 
            temperature : u16::MAX, 
    ...
    pub fn init(&mut self) {
        ...
        if who_am_i_val[0] != Lsm6dso::<SPI>::WHO_AM_I_DEF
        {
        ...
        ctrl1_xl_val[0] |= ((self.dev_conf.accelerometer.odr.clone() as u8) << Lsm6dso::<SPI>::ODR_BP) & Lsm6dso::<SPI>::ODR_BM;
        ctrl1_xl_val[0] |= ((self.dev_conf.accelerometer.full_scale.clone() as u8)<< Lsm6dso::<SPI>::FS_BP) & Lsm6dso::<SPI>::FS_BM;
        ...
        ctrl2_g_val[0] |= ((self.dev_conf.gyroscope.odr.clone() as u8)<< Lsm6dso::<SPI>::ODR_BP) & Lsm6dso::<SPI>::ODR_BM;
        ctrl2_g_val[0] |= ((self.dev_conf.gyroscope.full_scale.clone() as u8) << Lsm6dso::<SPI>::FS_BP) & Lsm6dso::<SPI>::FS_BM;
        self.write_reg(RegistersAddr::Ctrl2G, &ctrl2_g_val);

    }

    fn read_and_write_reg(&mut self, register_add : RegistersAddr, _data_in : &[u8], _data_out: &mut [u8]){

    }


    fn read_reg(&mut self,_register_add : RegistersAddr, _data_out: &mut [u8]){
        //first  : send reg and read bit
        let reg_cmd : [u8 ; 1] =  [Lsm6dso::<SPI>::READ_OP_BV | (_register_add as u8)];
        self.spi.write(&reg_cmd);
    }

    fn write_reg(&mut self,_register_add : RegistersAddr, _data_in : &[u8]){
    ...
    pub fn update_measures(&mut self){
        ...
        if status_reg_val[0] & (Lsm6dso::<SPI>::STATUS_REG_GDA_BM | Lsm6dso::<SPI>::STATUS_REG_XLDA_BM) == (Lsm6dso::<SPI>::STATUS_REG_GDA_BM | Lsm6dso::<SPI>::STATUS_REG_XLDA_BM)
           {
                ...
                self.angular_rate = Meas3d::from(angle_rate_val);
    ...
    pub fn get_temperature_degc(&self) -> u16{

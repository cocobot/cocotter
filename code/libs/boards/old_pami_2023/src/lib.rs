...
use vl53l5cx::VL53L5CXDevConfiguration;
...
pub type I2CVlxDevice = I2C0PamiDevice;
...
pub type TofType = Vl53l5cx<I2C0PamiDevice, 1>;
...
impl Pami2023 {
    pub fn new() -> Self {
        ...
        let (pwm_extended, line_sensor, buttons, tof) = Self::init_i2c_devices(i2c0);
        ...
        let adc = PamiAdc {
            ...
            i_mot_left: adc1_config.enable_pin_with_cal(peripherals.GPIO9, Attenuation::_0dB),
            i_mot_right: adc1_config.enable_pin_with_cal(peripherals.GPIO10, Attenuation::_0dB),
        ...
        Self {
            ...
            ble_connector: None,//Some(ble_connector),
        }
    }
    ...
    fn init_i2c_devices(
        ...
        //init vlx53l5cx
        let tof = Vl53l5cx::new(
            I2cDevice::new(i2c0),
            [
                VL53L5CXDevConfiguration::new(0x29)
            ]
        );

        (pwm_extended, line_sensor, buttons, tof)
    }

...
pub struct VL53L5CX {
    address: u8,
    configuration: Box<bindings::VL53L5CX_Configuration>,
}

impl VL53L5CX {
    pub fn new(address: u8) -> Self {
        // Initialize configuration with zeros - will be properly initialized by vl53l5cx_init
        let configuration = Box::new(bindings::VL53L5CX_Configuration::default());
...
impl Sensor for VL53L5CX {
    fn init(&mut self) -> Result<(), VlxError> {
        ...
        let status = unsafe { bindings::vl53l5cx_is_alive(&mut *self.configuration, &mut is_configured) };
        if status != 0 || is_configured == 0 {            
            self.configuration.platform.address = 0x29;
            // Check if sensor is alive
            let mut is_alive = 0u8;
            let status = unsafe { bindings::vl53l5cx_is_alive(&mut *self.configuration, &mut is_alive) };
            if status != 0 || is_alive == 0 {
                return Err(VlxError::InitError);
            }
            
            // Initialize sensor
            let status = unsafe { bindings::vl53l5cx_init(&mut *self.configuration) };
            if status != 0 {
                return Err(VlxError::InitError);
            }

            // Set I2C address if different from default
            let status = unsafe { bindings::vl53l5cx_set_i2c_address(&mut *self.configuration, (self.address * 2) as u16) };
            if status != 0 {
                return Err(VlxError::InitError);
            }
            self.configuration.platform.address = self.address as u16;
            ...
            let status = unsafe { bindings::vl53l5cx_set_resolution(&mut *self.configuration, 16) }; // VL53L5CX_RESOLUTION_4X4
            if status != 0 {
                return Err(VlxError::InitError);
            }
            
            // Set power mode to wake up
            let status = unsafe { bindings::vl53l5cx_set_power_mode(&mut *self.configuration, 1) }; // VL53L5CX_POWER_MODE_WAKEUP
            if status != 0 {
                return Err(VlxError::InitError);
            }
        }        
        ...
        let status = unsafe { bindings::vl53l5cx_start_ranging(&mut *self.configuration) };
        if status != 0 {
            return Err(VlxError::InitError);
        }
        Ok(())
    ...
    fn get_distance(&mut self) -> Result<DistanceData, VlxError> {
        ...
        let status = unsafe { bindings::vl53l5cx_check_data_ready(&mut *self.configuration, &mut is_data_ready) };
        if status != 0 {
        ...
        // Get ranging data
        let status = unsafe { bindings::vl53l5cx_get_ranging_data(&mut *self.configuration, &mut results) };
        if status != 0 {
            return Err(VlxError::RangingError);
        }
        ...
        if distances.is_empty() {
            ...
            let multizone = MultiZoneData::new(width, height, distances);
    ...
    fn set_alarm(&mut self, low: u16, high: u16, zone: Option<(usize, usize)>) -> Result<(), VlxError> {
        ...
        let status = unsafe { bindings::vl53l5cx_set_detection_thresholds_enable(&mut *self.configuration, 1) };
        if status != 0 {
        ...
        let status = unsafe { bindings::vl53l5cx_set_detection_thresholds(&mut *self.configuration, detection_thresholds.as_mut_ptr()) };
        if status != 0 {
            return Err(VlxError::ConfigError);
        }
    ...
    fn clear_alarm(&mut self) -> Result<(), VlxError> {
        ...
        let status = unsafe { bindings::vl53l5cx_set_detection_thresholds_enable(&mut *self.configuration, 0) };
        if status != 0 {
            return Err(VlxError::ConfigError);
        }
        Ok(())

...
where
{
    ...
    fn get_distance(&mut self) -> Result<crate::DistanceData, VlxError> {
        ...
        let status = unsafe { crate::VL53L1X_CheckForDataReady(self.address as u16, &mut is_data_ready) };
        if status != 0 {
        ...
        let status = unsafe { crate::VL53L1X_GetRangeStatus(self.address as u16, &mut range_status) };
        if status != 0 {
            return Err(VlxError::RangingError);
        }
        ...
        if range_status != 0 {
            ...
            let _ = unsafe { crate::VL53L1X_ClearInterrupt(self.address as u16) };
            return Err(VlxError::RangingError);
        }
        
        // Get distance measurement
        let status = unsafe { crate::VL53L1X_GetDistance(self.address as u16, &mut distance) };
        if status != 0 {
            return Err(VlxError::RangingError);
        }
        
        // Clear interrupt to arm for next measurement
        let status = unsafe { crate::VL53L1X_ClearInterrupt(self.address as u16) };
        if status != 0 {
            return Err(VlxError::RangingError);
        }
    ...
    fn set_alarm(&mut self, low: u16, high: u16, _zone: Option<(usize, usize)>) -> Result<(), VlxError> {
        ...
        let status = unsafe {
            crate::VL53L1X_SetDistanceThreshold(
                self.address as u16,

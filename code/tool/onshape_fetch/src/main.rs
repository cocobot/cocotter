...
struct Cli {
    ...
    /// Optional explicit raw-cache path. When set, the raw .glb is
    /// stored here instead of `<out>.raw` — share one cache across
    /// several invocations with different `--out` / filters to avoid
    /// re-downloading the same Onshape assembly.
    #[arg(long)]
...
fn main() -> Result<()> {
    ...
    let raw_path = cli.cache.clone().unwrap_or_else(|| raw_cache_path(&cli.out));

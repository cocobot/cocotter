...
/// Parse a pattern like `"Part 137##2"` into `("Part 137", Some(2))`.
/// Plain patterns like `"Part 137"` or `"*PCB*"` return `(pattern, None)`.
fn parse_instance_pattern(pat: &str) -> (&str, Option<usize>) {
    if let Some(pos) = pat.rfind("##") {
        if let Ok(n) = pat[pos + 2..].parse::<usize>() {
            return (&pat[..pos], Some(n));
        }
    }
    (pat, None)
}

/// Build include/exclude matchers that support instance indexing (`name##N`).
/// Returns two things:
/// - A GlobSet for plain patterns (no `##N`)
/// - A list of (glob, instance_index) for indexed patterns
fn build_instance_matchers(patterns: &[String]) -> Result<(GlobSet, Vec<(GlobSet, usize)>)> {
    let mut plain_builder = GlobSetBuilder::new();
    let mut indexed = Vec::new();
    for p in patterns {
        let (base, idx) = parse_instance_pattern(p);
        match idx {
            None => { plain_builder.add(Glob::new(base)?); }
            Some(n) => {
                let mut b = GlobSetBuilder::new();
                b.add(Glob::new(base)?);
                indexed.push((b.build()?, n));
            }
        }
    }
    Ok((plain_builder.build()?, indexed))
}

fn apply_node_filter(doc: &mut Value, include: &[String], exclude: &[String]) -> Result<()> {
    let (include_plain, include_indexed) = build_instance_matchers(include)?;
    let (exclude_plain, exclude_indexed) = build_instance_matchers(exclude)?;
    ...
    // Count occurrences of each name (for ##N matching)
    let mut name_counter: std::collections::HashMap<String, usize> = std::collections::HashMap::new();
    let mut node_instance: Vec<usize> = Vec::with_capacity(nodes.len());
    for n in &nodes {
        let name = n.get("name").and_then(|v| v.as_str()).unwrap_or("").to_string();
        let idx = name_counter.entry(name).or_insert(0);
        node_instance.push(*idx);
        *idx += 1;
    }
    ...
    for (i, n) in nodes.iter().enumerate() {
        ...
        let inst = node_instance[i];

        // Check exclude: plain glob OR indexed match
        let excluded = if !exclude.is_empty() {
            exclude_plain.is_match(name)
                || exclude_indexed.iter().any(|(gs, n)| gs.is_match(name) && inst == *n)
        } else {
            false
        };

        // Check include: plain glob OR indexed match
        let included = if include.is_empty() {
            true
        } else {
            include_plain.is_match(name)
                || include_indexed.iter().any(|(gs, n)| gs.is_match(name) && inst == *n)
        };

...
enum MockInputPinMode {
    /// Reads the latched side assignment from the sim client.
    /// Convention: `is_high` ⇔ Left, `is_low` ⇔ Right (matches the
    /// strat's `color.is_high() → Team::Left` mapping).
    Color,
    /// Reads the latched starter state from the sim client. Defaults
    /// to "not inserted" (`is_high`) until the sim broadcasts a
    /// `Starter { inserted: true }` frame (T pressed in the sim
    /// console, or the CLI auto-start sequence).
    Starter,
}

pub struct MockInputPin {
    mode: MockInputPinMode,
}

impl MockInputPin {
    pub fn color() -> Self {
        Self { mode: MockInputPinMode::Color }
    }

    pub fn starter() -> Self {
        Self { mode: MockInputPinMode::Starter }
    }
}
...
impl InputPin for MockInputPin {
    fn is_high(&mut self) -> Result<bool, Self::Error> {
        match self.mode {
            MockInputPinMode::Color => Ok(sim_client::try_global()
                .map(|c| c.is_side_left())
                .unwrap_or(true)),
            MockInputPinMode::Starter => Ok(!sim_client::try_global()
                .map(|c| c.is_starter_inserted())
                .unwrap_or(false)),
        }
    }
...
/// Mock WS2812 driver wrapped around the sim_client. Caches the last
/// frame and only forwards a `NeopixelFrame` when the pixel buffer
/// actually changed — the LED render thread on galipeur runs at
/// ~100 Hz, but most ticks reproduce the same buffer. Forwarding
/// every one used to push 41-pixel bincode frames over the IPC
/// socket 100 times per second, which alone pegged the galipeur
/// process around one full core.
#[derive(Default)]
...
impl SabotterBoard for MockSabotterBoard {
    ...
    fn init() -> Self {
        ...
        // The launcher (sim chord / `--spawn`) sets `SIDE=left|right`.
        // Forward it to the sim so it picks the right `start_poses.*`
        // entry and echoes the value back via `SimMsgS2C::Side`.
        let requested_side = match std::env::var("SIDE").ok().as_deref() {
            Some("left") => Some(sim_protocol::Side::Left),
            Some("right") => Some(sim_protocol::Side::Right),
            _ => None,
        };
        // Distinct id per spawn so the sim's world doesn't collide
        // when two galipeurs are running side-by-side. The launcher
        // (chord / `--spawn`) sets `MECA_SIM_ROBOT_ID`; default to a
        // side-suffixed name when running standalone via `cargo run`.
        let robot_id = std::env::var("MECA_SIM_ROBOT_ID").unwrap_or_else(|_| {
            match requested_side {
                Some(sim_protocol::Side::Right) => "galipeur-right".into(),
                _ => "galipeur".into(),
            }
        });
    ...
    fn leds(&mut self) -> Option<SabotterLeds<Self::OutputPin, Self::ExOutputPin, Self::SmartLeds>> {
        Some(SabotterLeds {
            ...
            rgba: MockSmartLeds::default(),
        })
    }

    fn inputs(&mut self) -> Option<SabotterInputs<Self::ExInputPin, Self::ExInputPin>> {
        Some(SabotterInputs {
            color: MockInputPin::color(),
            starter: MockInputPin::starter(),
        })
    }
    ...
    fn rome(&mut self, _device_name: String, mut _other_ota_handlers: Vec<Box<dyn OtaHandler>>) -> Option<(Sender<Box<[u8]>>, Sender<String>, Receiver<Box<[u8]>>)> {
        // Outgoing: caller sends, we drop. Incoming: never triggered (for M1).
        let (out_tx, out_rx) = flume::unbounded();
        let (_in_tx, in_rx) = flume::unbounded::<Box<[u8]>>();
        let (str_tx, str_rx) = flume::unbounded();
        thread::Builder::new()

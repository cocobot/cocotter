//! Online pose realignment using the ground lidars.
//!
...
//!
//! Geometric assumption (matches the user's
//! `MovementLowLevelHardware::teleport`):
//! - asserv +X axis ≡ direction from the Up wall toward the Down wall,
//! - asserv +Y axis ≡ direction from the Left wall toward the Right wall.
...
pub fn realign<B: SabotterBoard + 'static>(
    ...
    let po = sensors
        ...
        .ok_or(StrategyError::OpponentDetected)?;
    let current = asserv.position();

    let new_theta = wrap_pi(arfast(face, wall) - po.angle);

    // body sits at `wall_pos − distance` for "high-coordinate" walls
    // (Up at +X_max, Left at +Y_max) and `wall_pos + distance` for
    // the opposite walls.

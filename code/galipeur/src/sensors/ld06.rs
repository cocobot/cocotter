/// LD06 360-degree lidar protocol decoder

const HEADER: u8 = 0x54;
pub const PACKET_SIZE: usize = 47;
const POINTS_PER_REVOLUTION: usize = 500;
/// Minimum valid distance in mm. Points closer than this are likely
/// reflections from the robot body and are discarded.
const MIN_DISTANCE_MM: u16 = 125;

#[derive(Clone, Copy)]
pub struct LidarPoint {
    pub distance: u16, // mm
    pub intensity: u8,
}

#[allow(unused)]
pub struct LidarPacket {
    pub speed: u16,       // degrees/sec
    pub start_angle: f32, // degrees
    pub end_angle: f32,   // degrees
    pub points: [LidarPoint; 12],
    pub timestamp: u16,
}

/// Completed 360 scan snapshot
#[derive(Clone, Default, Debug)]
pub struct TopLidarSnapshot {
    pub points: Vec<(f32, u16, u8)>, // (angle_deg, distance_mm, intensity)
}

/// Accumulates LD06 packets into full revolutions
pub struct TopLidarScan {
    scan: Box<[(f32, u16, u8); POINTS_PER_REVOLUTION]>,
    scan_index: usize,
    last_angle: f32,
    angle_offset: f32, // degrees
}

impl TopLidarScan {
    pub fn new(angle_offset: f32) -> Self {
        Self {
            scan: Box::new([(0.0, 0, 0); POINTS_PER_REVOLUTION]),
            scan_index: 0,
            last_angle: 0.0,
            angle_offset,
        }
    }

    /// Process one LD06 packet. Returns a snapshot when a full revolution completes.
    pub fn process_packet(&mut self, packet: &LidarPacket) -> Option<TopLidarSnapshot> {
        let new_revolution = packet.start_angle < self.last_angle - 180.0;
        let mut result = None;

        if new_revolution && self.scan_index > 0 {
            let points = self.scan[..self.scan_index].to_vec();
            result = Some(TopLidarSnapshot { points });
            self.scan_index = 0;
        }

        self.last_angle = packet.start_angle;

        let angle_step = if packet.end_angle >= packet.start_angle {
            (packet.end_angle - packet.start_angle) / 11.0
        } else {
            (packet.end_angle + 360.0 - packet.start_angle) / 11.0
        };

        for (i, point) in packet.points.iter().enumerate() {
            if point.distance > MIN_DISTANCE_MM && self.scan_index < POINTS_PER_REVOLUTION {
                let angle = (self.angle_offset - packet.start_angle - angle_step * i as f32).rem_euclid(360.0);
                self.scan[self.scan_index] = (angle, point.distance, point.intensity);
                self.scan_index += 1;
            }
        }

        result
    }
}

/// Extract (angle_deg, distance_mm, intensity) tuples from a single packet.
/// Applies the same angle offset as TopLidarScan.
pub fn packet_points(packet: &LidarPacket, angle_offset: f32) -> [(f32, u16, u8); 12] {
    let angle_step = if packet.end_angle >= packet.start_angle {
        (packet.end_angle - packet.start_angle) / 11.0
    } else {
        (packet.end_angle + 360.0 - packet.start_angle) / 11.0
    };

    let mut out = [(0.0f32, 0u16, 0u8); 12];
    for (i, point) in packet.points.iter().enumerate() {
        let angle = (angle_offset - packet.start_angle - angle_step * i as f32).rem_euclid(360.0);
        let dist = if point.distance > MIN_DISTANCE_MM { point.distance } else { 0 };
        out[i] = (angle, dist, point.intensity);
    }
    out
}

/// CRC8 lookup table for polynomial 0x4D
const CRC_TABLE: [u8; 256] = [
    ,
];

pub fn verify_crc(data: &[u8; PACKET_SIZE]) -> bool {
    let mut crc: u8 = 0;
    for &byte in &data[..PACKET_SIZE - 1] {
        crc = CRC_TABLE[(crc ^ byte) as usize];
    }
    crc == data[PACKET_SIZE - 1]
}

pub fn parse_packet(data: &[u8; PACKET_SIZE]) -> Option<LidarPacket> {
    if data[0] != HEADER || data[1] != 0x2C {
        return None;
    }

    let speed = u16::from_le_bytes([data[2], data[3]]);
    let start_angle = u16::from_le_bytes([data[4], data[5]]) as f32 * 0.01;
    let end_angle = u16::from_le_bytes([data[42], data[43]]) as f32 * 0.01;
    let timestamp = u16::from_le_bytes([data[44], data[45]]);

    let mut points = [LidarPoint { distance: 0, intensity: 0 }; 12];
    for i in 0..12 {
        let base = 6 + i * 3;
        points[i] = LidarPoint {
            distance: u16::from_le_bytes([data[base], data[base + 1]]),
            intensity: data[base + 2],
        };
    }

    Some(LidarPacket {
        speed,
        start_angle,
        end_angle,
        points,
        timestamp,
    })
}

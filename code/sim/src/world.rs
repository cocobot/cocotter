...
pub struct World {
    ...
    /// Mirror of the Bevy `HumansVisible` toggle. When false, humans are
    /// excluded from `visible_segments` so the lidar/VLX raycasts don't
    /// hit invisible bodies. Defaults to `false` to match the Bevy
    /// `HumansVisible(false)` initial state.
    humans_visible: Arc<AtomicBool>,
}
...
impl World {
    ...
    /// Snapshot of every entity currently in the world. Used by HUD
    /// systems that want to display the live robot list.
    pub fn entries(&self) -> Vec<(String, EntitySnapshot)> {
        self.inner
            .lock()
            .unwrap()
            .iter()
    ...
    pub fn set_humans_visible(&self, v: bool) {
        self.humans_visible.store(v, Ordering::Relaxed);
    }

    pub fn humans_visible(&self) -> bool {
        self.humans_visible.load(Ordering::Relaxed)
    }
    ...
    pub fn visible_segments(
        ...
        let humans_visible = self.humans_visible();
        let guard = self.inner.lock().unwrap();
        for (id, s) in guard.iter() {
            if id == skip_id {
                continue;
            }
            // Hidden humans must also disappear from raycast — otherwise
            // the lidar bumps into invisible silhouettes the operator
            // already toggled away with `C`.
            if !humans_visible && matches!(s.kind, EntityKind::Human) {
                continue;
            }
